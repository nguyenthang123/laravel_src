...
class AdminController extends Controller {
	public function login(){
        ...
        if(isset($_POST['login'])){
            ...
            $arr=array('adm_name'=>$adm_name,'adm_password'=>$adm_password);
    ...
    public function postAdd(ValAdminAdd $request){
        ...
        $dulieu_form['adm_rewrite']=$dulieu_form['adm_name'];
        ...
        //check admin name
        $arr=array('adm_name'=>$dulieu_form['adm_name'],'adm_rewrite'=>$dulieu_form['adm_name']);
        ...
        if(!empty($data)){
            ...
            if($rs==true){
                Admin::logs('admin/postAdd | Bạn đã thêm thành viên | '.$dulieu_form['adm_name']);
    ...
    public function update($id,ValAdminEdit $request){
        ...
        if($rs==true){
            Admin::logs('admin/update | Bạn đã sửa thành viên | id='.$id.' | '.$form['adm_name']);
            $funs->redirect(domain.'/admin/admin/index');
        }else{
    ...
    public function del($id=null){
        ...
        if(!empty($id)){
            ...
            if($rs>0){
                Admin::logs('admin/del | Bạn đã xóa thành viên | id='.$id);
