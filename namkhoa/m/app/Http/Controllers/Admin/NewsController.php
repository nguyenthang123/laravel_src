...
class NewsController extends Controller {
    ...
    public function postAdd(ValNewsAdd $request){
        ...
        if(!empty($data)){
            ...
            if($rs==true){
                ...
                DB::logs('news/postAdd | Bạn đã thêm bài viết | '.$id_max['id'].'-'.$form['new_name']);
    ...
    public function update($id,ValNewsAdd $request){
        ...
        if(empty($form['new_xetnghiem'])){
            $form['new_xetnghiem']='0';
        }
        if(empty($form['new_thoigian'])){
            $form['new_thoigian']='0';
        }
        if(empty($form['new_thuthuat'])){
            $form['new_thuthuat']='0';
        }
    ...
    public function index($cate=null){
        ...
        //$count = DB::table('news')->whereDate('created_at', '=','2016-04-26')->count();
        //echo $count;
        manage(domain.'/admin');
