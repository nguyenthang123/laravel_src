...
class NewsController extends Controller {
    ...
    public function chinhhinhcoquansinhduc(){
        //get category parent
        $Arr=array('cat_rewrite'=>'chinh-hinh-co-quan-sinh-duc');
        $cate = DB::table('categories')
            ->select('id','cat_name','cat_img','cat_img_style','cat_seotitle','cat_seokeywords','cat_seodescription')
            ->where($Arr)->first();
        //get danh muc
    ...
    public function tienliettuyen(){
        //get category parent
        $Arr=array('cat_rewrite'=>'tien-liet-tuyen');
        $cate = DB::table('categories')
            ->select('id','cat_img','cat_img_style')
            ->where($Arr)->first();
        //get danh muc
        $Arr = array('cat_parent' => $cate['id'],'cat_status'=>1);
    ...
    public function vosinhnam(){
        $Arr=array('cat_rewrite'=>'vo-sinh-nam');
        $cate = DB::table('categories')->where($Arr)->first();
        //khong tinh trung
