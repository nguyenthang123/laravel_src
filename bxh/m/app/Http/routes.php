...
//$arrNotCate=array(cat_rewrite_yeusinhly,cat_rewrite_tienliettuyen,cat_rewrite_viemnhiemcoquansinhduc,cat_rewrite_hepbaoquydau,cat_rewrite_vosinh,cat_rewrite_benhxahoi);
if(empty($_SESSION['visit'])){
    $Arr=array('cat_status'=>1);
    $cate=DB::table('categories')->where($Arr)->get();
    $ArrCate=array();
    $ArrCateShowList=array();
    $ArrCateShowNn=array();
    foreach($cate as $val){
        if(!empty($val['cat_rewrite'])){
            array_push($ArrCate,$val['cat_rewrite']);
        }
        if($val['cat_type_show']=='list'){
            array_push($ArrCateShowList,$val['cat_rewrite'].'_list');
        }
        if($val['cat_type_show']=='nguyennhan'){
            array_push($ArrCateShowNn,$val['cat_rewrite'].'_nguyennhan');
        }
    }
    $_SESSION['ArrCate']=$ArrCate;
    $_SESSION['ArrCateShowList']=$ArrCateShowList;
    $_SESSION['ArrCateShowNn']=$ArrCateShowNn;
    $_SESSION['visit']="1";
}
...
if(in_array($ext,$_SESSION['ArrCate'])){
    if(in_array($ext.'_list',$_SESSION['ArrCateShowList'])){
        Route::any('/{rewrite}', 'Frontend\NewsController@showlist');
    }elseif(in_array($ext.'_nguyennhan',$_SESSION['ArrCateShowNn'])){
...
Route::get('/{dmbenh}/{rewrite}', 'Frontend\NewsController@dmbenh');
//=========ADMIN=============
