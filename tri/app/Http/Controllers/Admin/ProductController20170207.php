...
class ProductController extends Controller {
	...
	public function create()
	{
		...
		return view('admin.product.create')->with('all_products',$all_products)->with('all_products_0',$all_products_0)->with('all_products_1',$all_products_1)->with('cats',$cats)->with('cat_parents',$cat_parents)->with('admin_changes',$admin_changes);
	}
...
//exit();
	...
	public function thay_doi_title() {
		$products = App\Product::all();
		//var_dump($products);
		echo "<h1>".count($products)." bài viết </h1>";
		foreach($products as $product) {
			$product = App\Product::find($product->id);
		//	$product->product_alias=strtolower(convert_vi_to_en($product->name));
		//	$product->save();
		
			echo "<a href=".$product->product_alias."> $product->name</a>" ;
			echo "</br>";
		}
		exit();
		for ($i=1150; $i <1250 ; $i++) {
